...
export async function POST(request: NextRequest) {
	try {
		...
		if (!badge) {
			...
			console.error(`Requested badge ID (hex):`, Array.from(trimmedBadgeId).map(c => c.charCodeAt(0).toString(16)).join(' '));
			...
			console.error(`Available badge IDs (hex):`, allCompanyBadges.map(b => Array.from(b.id).map(c => c.charCodeAt(0).toString(16)).join(' ')));
